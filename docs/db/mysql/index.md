...
索引是一种数据结构,主要用于提高数据检索效率。它的工作原理类似于书籍的目录,让数据库能在大规模数据集中迅速找到所需的信息。

MySQL 官方对索引的定义是:索引(Index)是帮助 MySQL 高效获取数据的数据结构。可以简单理解为一种排序的支持快速查找的数据结构,这些数据结构以某种方式指向具体数据,通过高级查找算法可以实现快速查找。在 MySQL 中,索引是在存储引擎层面实现的,因此不同存储引擎的索引各有差异,并且每种存储引擎不一定支持所有索引类型。同时,存储引擎可以定义表的最大索引数和最大索引长度。所有存储引擎支持每个表至少 16 个索引,总索引长度至少为 256 字节,有些存储引擎支持更大的最大索引数和最大索引长度。
