...
public class DogExample {
    ...
    public static void main(String[] args) {
        ...
        /**
         * System.out.println打印对象时,会自动调用该对象
         * 的toString()方法来获取字符串表示形式并打印。
         */
        System.out.println(dog); // Dog{name=大黄, age='10'}
    }
}
```
...
在 Java 中,public、private、protected 是访问修饰符(Access Modifiers),用于控制类的成员(字段、方法、构造函数)的访问级别和可见性。它们具有不同的作用和限制,用于确保封装性、代码安全性和灵活性。

- public:
  - public 是最开放的访问级别,可以从任何地方访问被修饰的成员。当一个成员被声明为 public 时,它可以被该类的所有方法、其他类和其他包中的代码访问。
  - public 修饰的成员在类的外部是可见的,可以被其他类实例化、调用和访问。当不写修饰符时默认使用 public。
- private:
  - private 是最严格的访问级别,只能在当前类内部访问。当一个成员被声明为 private 时,它只能在声明该成员的类的内部使用,其他类无法直接访问该成员。
  - private 修饰的成员对于类的外部是不可见的,可以在类的内部进行操作和访问。
- protected:
  - protected 允许在当前类、同一包中的其他类以及其他包中的子类访问。当一个成员被声明为 protected 时,它对于声明该成员的类的子类和同一包中的其他类是可见的,但对于其他包中的类是不可见的。
  - protected 修饰的成员对于其他包中的类是不可访问的,但对于子类和同一包中的其他类是可见的。
