...
在 Java 中,虽然万物皆对象,但为了优化性能和内存使用,Java 将数据类型分为基本类型(Primitive Types)和引用类型(Reference Types):
...
- **默认值和分配空间不同**:Java 中的基本类型是直接存储数据值的简单类型,它们具有固定的大小和默认值,并且在栈(Stack)上分配内存。因此,基本类型的存储效率高,因为它们直接存储数据值,不需要额外的内存空间来存储对象头信息或引用。引用类型的变量在栈中存储的是对象的引用(地址),而对象本身则存储在堆(Heap)中,默认值都是 null。引用类型在堆上分配内存,因此可能需要更多的内存空间,并且在访问时需要通过引用来间接访问对象。
- **存储方式和访问效率不同**:由于基本类型直接存储数据值,因此访问速度通常比引用类型更快。此外,基本类型不会涉及垃圾回收(Garbage Collection)的问题,因为它们不会动态分配或释放堆内存。引用类型的对象存储在堆内存中,由 Java 虚拟机的垃圾回收器管理。这种方式提供了更灵活的内存管理,但也会带来性能上的一些开销,如内存分配、垃圾回收和对象访问的间接性。
...
在 Java 中,== 和 equals() 是用来比较对象的方法,但它们之间有着不同的作用和使用场景:

- == 操作符:== 是一个操作符,用于比较两个对象的引用(内存地址)是否相同。如果两个对象的引用指向的是同一个内存地址(即它们是同一个对象),则 == 返回 true。对于基本类型,== 用于比较它们的值是否相等。
- equals() 是 Object 类中定义的方法,用于比较两个对象的内容是否相等。子类可以根据需要重写 equals() 方法来定义对象相等的条件。默认情况下,equals() 方法在 Object 类中的实现是使用 == 操作符来比较两个对象的引用是否相同,因此默认行为和 == 是一样的。
...
System.out.println(s1 == s2); // false,s1和s2都是通过new创建的字符串,内存地址不同
System.out.println(s1.equals(s2)); // true,s1和s2的字符串内容是相同的
System.out.println(s2 == s3); // true
/**
 * 说明:String.intern()是一个本地方法,用于返回字符串对象的规范化表示形式。也就是说,它会返回字符串池(String Pool)
 * 中的唯一实例,如果池中已经存在该字符串,则返回池中的引用；如果池中不存在该字符串,则将该字符串添加到池中,
 * 并返回对该字符串的引用。
 */
```
...
使用 new String("abc") 这种方式创建了两个对象:

- 常量池中的字符串对象:"abc" 字符串常量会首先在常量池中创建,如果常量池中已经存在相同内容的字符串,则不会重复创建,而是直接返回引用。
- 堆中的字符串对象:使用 new String("abc") 显式地创建了一个新的字符串对象,这个对象存储在堆内存中。即使常量池中已经有了 "abc" 字符串对象,使用 new String("abc") 仍然会在堆内存中创建一个新的对象。
  所以,总结来说,new String("abc") 创建了两个对象:一个是常量池中的字符串对象 "abc",另一个是堆中的新的字符串对象。
...
- **字符串压缩**:JDK 9 引入了一个名为 COMPACT_STRINGS(压缩字符串)的新特性。它允许 String 类在某些情况下以较低的内存开销存储字符串。具体来说,当字符串仅包含拉丁字母(Latin-1 字符集)时,String 类使用单字节存储每个字符,从而节省了内存。
- **其他改进**:JDK 9 还对字符串类进行了其他一些改进,包括更好的 Unicode 支持、更高效的 substring 操作、更好的正则表达式性能等。

## String、StringBuffer、StringBuilder的区别?

- String:不可变。每次拼接都会生成新对象,浪费内存。
- StringBuilder:可变。效率最高,但线程不安全(单线程推荐)。
- StringBuffer:可变。方法加了锁(依赖于synchronized实现,StringBuffer修改数据的方法都使用synchronized 修饰符以保证线程安全),线程安全,但效率略低。
...
在 Java 中,Object 类是所有类的“祖宗”(根类)。这意味着 Java 中的每一个类都默认继承了 Object 的方法。Object类包含以下方法:

- getClass():获取对象的运行时 Class 对象(反射入口)。
- equals(Object obj):判断两个对象是否“相等”(默认比较地址)。
- hashCode():返回对象的哈希码值(用于 HashMap 等集合)。
- toString():返回对象的字符串表示(默认:类名@十六进制哈希)。
- clone():创建并返回对象的一个副本(深/浅拷贝需注意)。Object.clone() 是一个 native 方法,默认是浅拷贝(Shallow Copy)。如果对象内部引用了其他对象,克隆后的副本和原对象会指向同一个内部对象。实现深拷贝通常需要手动递归克隆,或者通过序列化/反序列化来实现。
- wait()/notify()/notifyAll():这三个方法用于线程之间的通讯。wait()用于等待线程,notify()用于唤醒线程。notifyAll()用于唤醒所有在该对象监视器上等待的线程。由于Java 的锁是 “管程(Monitor)” 模型,每一把锁都是对象级别的(对象头里的 Mark Word 存储了锁状态),因此将wait/notify方法封装到Object类。
- finalize:垃圾回收前的“临终遗言”（已废弃，不建议使用）
...
- `equals()`:是 Object 类的方法。默认情况下它和 == 是一样的。但大多数类(如 String、Integer)都重写了这个方法,用来比较对象的内容是否相等。

## 什么是面向对象?

面向对象(Object-Oriented Programming, 简称 OOP)是一种编程思想。它不再关注“第一步做什么,第二步做什么”(那是过程导向),而是关注 **“由谁来做”**。在 OOP 的世界里,将现实世界中的事物抽象成 ** “对象(Object)”。每个对象包含了自己的数据（属性）和行为（方法）**。面向对象四大特性:

- 封装:封装是将数据(属性)和操作数据的逻辑(方法)捆绑在一起,并隐藏对象的内部实现细节,仅对外暴露有限的接口。封装可以保证数据的安全性,防止外部直接修改核心数据,其次可以降低耦合度,当内部逻辑改变时,只要接口不变,外部调用者不需要改代码。
- 继承:继承允许创建一个新类(子类)来复用现有类(父类)的属性和方法,并在此基础上进行扩展。继承可以提升复用性,并为子类提供了统一的基类,但是过度使用继承会导致类层级过深,此时可以基于 **“组合优于继承”**思想将依赖组合到一个类中。
- 多态:多态是指同一个方法调用,由于对象实例的不同,可以产生不同的执行结果。继承是多态的基础,没有继承(或接口实现),多态就无法实现。多态要求必须存在一个父类引用指向子类对象的场景,可以极大地提升类的扩展性。
- 抽象:抽象是将一类事物的共同特征提取出来,构造一个通用的模板,而不关注具体的实现。在Java中一般通过接口(Interface)和抽象类(abstract)实现,抽象是接口与实现分离的基础。架构设计中,面向接口编程的核心就是利用抽象。

## 接口 (Interface) 和 抽象类 (Abstract Class) 的区别？

- 抽象类:是对“类”的抽象(IS-A 关系),可以有成员变量和普通方法。
- 接口:是对“行为”的抽象(LIKE-A 关系),Java 8 之前只能有常量和抽象方法,Java 8 之后可以有 default 方法。
...
在 Java 中,BIO、NIO 和 AIO 是三种不同的 I/O 模型,分别代表阻塞 I/O(Blocking I/O)、非阻塞 I/O(Non-blocking I/O)和异步 I/O(Asynchronous I/O)。

- BIO(Blocking I/O):传统的 I/O 模型,也称为同步 I/O。在进行 I/O 操作时,如果数据没有准备好或无法立即处理,线程会被阻塞,直到数据准备好或超时。在阻塞 IO 模型中,每个 I/O 操作都会阻塞线程,线程会等待数据准备完毕。BIO 虽然简单易用,但在高并发环境下性能较差,因为需要大量线程阻塞等待 I/O 完成。例如早期的 Servlet 在处理 HTTP 请求时通常会使用阻塞 I/O 模型,每个请求到达时,Servlet 容器会为其分配一个线程来处理请求和生成响应。
- NIO(Non-blocking I/O):NIO 是在 Java 1.4 版本中引入的新特性,Java NIO 提供了一种更为灵活和高效的 I/O 处理方式,适用于需要处理大量并发连接的网络应用。在 NIO 中引入了选择器(Selector)和通道(Channel)的新 I/O 模型,可以实现非阻塞式的 I/O 操作。例如 Tomcat、Jetty、Netty 等,它们利用 NIO 来处理大量的客户端连接请求,提高并发处理能力。
- 异步 I/O:最新的 I/O 模型,也称为 NIO.2,引入了异步通道(AsynchronousChannel)和异步流(AsynchronousStream)。AIO 提供了更高级别的异步 I/O 操作,不需要通过轮询等待数据准备好,可以在数据准备好后自动通知应用程序,适合处理大量并发的 I/O 操作,例如高性能的服务器应用。
