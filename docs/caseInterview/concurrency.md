...
并行(Parallelism)和并发(Concurrency)是两个在计算机领域经常讨论的概念,它们描述了程序或系统中处理多个任务的不同方式:

- 并行(Parallelism):并行指的是同时执行多个任务或操作,通常是指在多核处理器或多处理器系统上同时执行多个线程或进程。在并行处理中,每个任务被划分为更小的子任务,并且这些子任务可以同时执行,从而加速整体的处理速度。
- 并发(Concurrency):并发指的是系统具备同时处理多个任务的能力,但不一定是同时执行。在并发处理中,任务可能是交替执行的,通过任务切换(Context Switching,即上下文切换)来实现看似同时执行的效果。多个任务可以在一段时间内交替执行,但每个时刻只有一个任务在执行。
...
## 6.Runnable 与 Callable 的区别?

Callable 接口与 Runnable 接口创建线程的区别在于:

- 重写方法不同。Runnable 接口创建线程需要重写 run(),Callable 接口创建线程需要重写 call()。
- 方法返回值不同。Runnable 接口的 run()无返回值,Callable 接口的 call()有返回值,返回值类型取决于 Callable 接口的泛型参数。当任务执行完成后,可以通过 Future 对象来获取任务的返回结果,或者阻塞等待任务完成并获取结果。
- call()内部支持抛出异常,run()内部不支持抛出异常。

## 7.Thread 的 start()与 run()的区别?
...
## 18.synchronized 的实现原理?

## 19.synchronized 锁升级过程?

## 20.synchronized 与 JUC 显式锁的区别?
